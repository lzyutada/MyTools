...
namespace C_WMS.Data.CWms.Interfaces.Data
{
    ...
    class HttpReqXml_StockoutOrderCreate_DeliveryOrder : Interface.CWms.Interfaces.Data.HttpXmlBase
    {
        ...
        public string CopyFrom(CWmsExWarehouseOrder srcObj)
        {
            if (null == srcObj)
            ...
            deliveryOrderCode = (srcObj.MangoOrder as MangoExwarehouseOrder).ProductOutputMainId.ToString();
            orderType = srcObj.WmsStockoutType.ToString(); //  ret.Append("wms出库单类型:{0}", srcObj.WmsStockoutType);
            warehouseCode = srcObj.Handler.GetWarehouse(srcObj).WarehouseCode;
            ...
            partialShipment = CWmsConsts.cStrStockoutPartialShipmentDefault;
            logisticsCode = logistics?.logisticsName;   // 承运商Code
            logisticsName = logistics?.logisticsName;   // 承运商名称

            receiverInfo.CopyFrom(new CWmsAgentBase(TCwmsAgentType.ESHR, (srcObj.MangoOrder as MangoExwarehouseOrder).JieShouRen.ToString()));
            senderInfo.CopyFrom(new CWmsAgentBase(TCwmsAgentType.EFHR, (srcObj.MangoOrder as MangoExwarehouseOrder).ChuKuRen.ToString()));
    ...
    class HttpReqXml_StockoutOrderCreate_OrderLine : Interface.CWms.Interfaces.Data.HttpXmlBase
    {
        ...
        /// <returns>若成功则返回string.Empty; 否则返回错误描述</returns>
        public string CopyFrom(CWmsExWarehouseSubOrder pSrcObj)
        {
            //ret.Append("CopyFrom(), 检查应收数量");
            if (null == pSrcObj)
                return "HttpReqXml_StockoutOrderCreate_OrderLine.CopyFrom异常，非法入参srcObj";

            var mseo = pSrcObj.MangoOrder as MangoSubExwarehouseOrder;

            outBizCode = pSrcObj.WmsOrderDetail.OutBizCode;
            orderLineNo = mseo.ProductOutputId.ToString();
            ownerCode = pSrcObj.WmsOrderDetail.Owner.WmsID;    // TODO: 在获取子出库单时获取货主
