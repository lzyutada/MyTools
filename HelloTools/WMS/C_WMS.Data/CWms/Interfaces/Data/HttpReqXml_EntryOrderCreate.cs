...
namespace C_WMS.Data.CWms.Interfaces.Data
{
    ...
    class HttpReqXml_EntryOrderCreate_EntryOrder
    {
        ...
        public string CopyFrom(CWmsEntryOrder pSrc)
        {
            ...
            try
            {
                ...
                ownerCode = MangoFactory.GetOwner(pSrc.MangoOrder.WarehouseId.ToString()).WmsID; // TODO: 根据仓库Id判断货主
                purchaseOrderCode = string.Empty; // 无法确定采购订单Id（多个主采购订单的子单据都可以加到一个主入库单中）
                warehouseCode = CWmsConsts.cStrDefaultWarehouseId;  // TODO: get warehous ID
    ...
    class HttpReqXml_EntryOrderCreate_Order
    {
        ...
        public string CopyFrom(CWmsSubEntryOder srcOrder)
        {
            ...
            try
            {
                outBizCode = (srcOrder.WmsOrderDetail).OutBizCode; // 外部业务编码, 消息ID, 用于去重，当单据需要分批次发送时使用
                orderLineNo = srcOrder.GetId(); // 入库单的行号
                ownerCode = MangoFactory.GetOwner(srcOrder.MangoOrder.WarehouseId.ToString()).WmsID; // 根据仓库Id判断货主
                itemCode = srcOrder.Product.ItemCode; // ret.Append("itemCode={0}", itemCode); // 商品编码
                itemName = srcOrder.Product.MangoProduct.Title; // 商品名称
                planQty = (srcOrder.WmsOrderDetail as Wms.Data.WmsEntryOrderDetail).planQty.ToString(); // 获取应收数量
                inventoryType = srcOrder.WmsOrderDetail.InventoryType.ToString();
