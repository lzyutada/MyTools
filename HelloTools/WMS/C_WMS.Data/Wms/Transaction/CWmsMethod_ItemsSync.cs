...
namespace C_WMS.Data.Wms.Transaction
{
    ...
    /// <summary>
     /// 库存同步接口
     /// </summary>
    class CWmsMethod_ItemsSync : CWmsPostTransactionBase
    {
...
#pragma warning restore CS1591 // 缺少对公共可见类型或成员的 XML 注释
    ...
    #region Properties
        ...
        /// 当前异步状态
        /// </summary>
        public TAsyncStatus AsyncStatus { get { return AsyncStatus; } }
        protected TAsyncStatus AsyncStatus = TAsyncStatus.EStopped;
    #endregion
    ...
    #region delegates
                ...
                case TAsyncStatus.EStopped:
                    {
                        ...
                        AsyncStatus = TAsyncStatus.EBegin;
                        err = TError.RunGood.Int();
                        break;
                    }
                default: { break; }
            }
            //ret.Append("CWmsMethod_ItemsSyncCtrl.Activate. Try to activate, AsyncStatus={0}, err={1}", AsyncStatus, err);
            return err;
        }
        ...
        {
            //ret.Append("DoRunL({0})开始", AsyncStatus);
                ...
                case TAsyncStatus.EBegin:               // 开始异步会话
                    {
                        ...
                        AsyncStatus = (0 < err) ? TAsyncStatus.EResetDict709Start : TAsyncStatus.EFailed;
                        err = TError.RunGood.Int();
                        break;
                    }
                case TAsyncStatus.EResetDict709Start:   // 开始重置Dict709
                    {
                        ...
                        AsyncStatus = (TError.RunGood.Int() == err) ? TAsyncStatus.EResetDict709Update : TAsyncStatus.EFailed;
                        break;
                    }
                case TAsyncStatus.EResetDict709Update:  // 继续更新Dict709中的行
                    {
                        ...
                        if (1 == err)
                            AsyncStatus = TAsyncStatus.EResetDict709Update;    // 更新成功，继续
                        else if (0 == err)
                            AsyncStatus = TAsyncStatus.EResetDict709Add;       //　无待更新商品，开始插入新行
                        else
                        {
                            AsyncStatus = TAsyncStatus.EFailed;                // 系统错误，执行失败
                        }
                        err = TError.RunGood.Int();
                        break;
                    }
                case TAsyncStatus.EResetDict709Add:     // 继续添加Dict709中的行
                    {
                        ...
                        if (1 == err)
                            AsyncStatus = TAsyncStatus.EResetDict709Add;       // 插入成功，继续
                        else if (0 == err)
                            AsyncStatus = TAsyncStatus.EResetDict709Success;   //　无待插入商品，重置709完成
                        else
                        {
                            AsyncStatus = TAsyncStatus.EFailed;                // 系统错误，执行失败
                        }
                ...
                case TAsyncStatus.EResetDict709Success:        // 重置Dict709完成
                    {
                        ...
                        err = TError.RunGood.Int();
                        AsyncStatus = TAsyncStatus.EGetBarCode;
                        break;
                    }
                case TAsyncStatus.EGetBarCode:    // 获取商品的条形码
                    {
                        ...
                        if (1 == err)
                        {
                            AsyncStatus = TAsyncStatus.EGetBarCode;       // 插入获取当前商品条码，继续
                            pHandler.CurrentProductItemGoNext();
                        }
                        else if (0 == err) // get owners & update asyncstatus
                            AsyncStatus = (0 >= pHandler.GetOwners()) ? TAsyncStatus.EFailed : TAsyncStatus.EReadyForSync;   //　无待获取条码的商品，获取完成
                        else
                            AsyncStatus = TAsyncStatus.EFailed;                // 系统错误，执行失败
                        err = TError.RunGood.Int();
                        break;
                ...
                case TAsyncStatus.EReadyForSync:    // 开始同步商品前需要做的事情
                    {
                        ...
                        AsyncStatus = TAsyncStatus.EComposeReqXml;
                        err = TError.RunGood.Int();
                        break;
                    }
                case TAsyncStatus.EComposeReqXml: // 针对当前货主组装RequestXML内容，对每个货主都同步所有的待同步商品
                    {
                        ...
                        if (1 == err)
                        {
                            AsyncStatus = TAsyncStatus.EDoTrans;       // 成功获取Request实体，继续
                            err = TError.RunGood.Int();
                        }
                        else if (0 == err)
                        {
                            ...
                            AsyncStatus = (TError.RunGood.Int() == err)?TAsyncStatus.EUpdateDict709: TAsyncStatus.EFailed;
                        }
                        else
                        {
                            AsyncStatus = TAsyncStatus.EFailed;                // 系统错误，执行失败
                ...
                case TAsyncStatus.EUpdateDict709: // 更新Dict709
                    {
                        err = pHandler.UpdatePwiItem(out errMsg);
                        if (1 == err)
                            AsyncStatus = TAsyncStatus.EUpdateDict709;    // 更新成功，继续
                        else if (0 == err)
                            AsyncStatus = TAsyncStatus.ESuccess;       //　无待更新商品，开始插入新行
                        else
                            AsyncStatus = TAsyncStatus.EFailed;                // 系统错误，执行失败
                        err = TError.RunGood.Int();
                        break;
                ...
                case TAsyncStatus.ESuccess:     // WMS同步操作成功
                    {
                        ...
                        err = TError.RunGood.Int();
                        AsyncStatus = TAsyncStatus.EStopped;
                        break;
                    }
...
#else
    class CWmsTransactionItemSync : MWmsTransactionBase<CWmsTransactionItemSync, HttpReqXml_ItemsSyncronize, HttpRespXml_ItemsSynchronize>
    {
        protected override IMWmsTransactionImpl<HttpReqXml_ItemsSyncronize, HttpRespXml_ItemsSynchronize> InitImpl()
        {
            return new CWmsTransactionItemSyncImpl();
        }
    }

    class CWmsTransactionItemSyncImpl : CWmsTransactionImplBase<HttpReqXml_ItemsSyncronize, HttpRespXml_ItemsSynchronize>
    {
        public enum TAsyncStatus
        {
            EBegin,         // 开始异步会话
            EGetPidList,    // get list of id of products to be synchronizing.
            EParseArguments,    // parse argument(list of id )
            EReset709,          // reset IsUpdateOK for all products
            ENewRequestObject,  // new an object of requset
            EDoTransaction, // executing transaction
            EUpdate709,     // update 709 according to response from HTTP Transaction
            ESuccess,       // WMS同步操作成功
            EFailed,        // WMS同步操作失败
            EStopped        // 结束异步会话
        }
        ...
        /// 当前异步状态
        /// </summary>
        public TAsyncStatus AsyncStatus { get; protected set; }
        protected object _asyncArg = null;  // product id list

        /// <summary>
        /// 待同步的商品的ID列表。
        /// 若ProductList为null，则视为为初始化，不能执行后续的同步操作；
        /// 若ProductList不为null, 但其Count为0，则执行同步全部商品的操作；
        /// 否则同步ProductList中所有的商品。
        /// key - product ID; value - barcode
        /// </summary>
        Dictionary<string, string> ProductIdList = null;

        /// <summary>
        /// WMS系统中全部货主的列表
        /// </summary>
        List<CWmsOwner> OwnerList = null;

        /// <summary>
        /// request objects to be sent
        /// </summary>
        List<HttpReqXml_ItemsSyncronize> ReqObjList { get { return (null == _reqObjList) ? _reqObjList = new List<HttpReqXml_ItemsSyncronize>(1) : _reqObjList; } }
        List<HttpReqXml_ItemsSyncronize> _reqObjList = null;

        HttpRespXml_ItemsSynchronize ResponseObject = null;

        /// <summary>
        /// default constructor
        /// </summary>
        public CWmsTransactionItemSyncImpl()
        {
            OwnerList = CWmsDataFactory.GetV_Owners().ToList(); // get all owners in WMS
        }

        /// <summary>
        /// 判断是否从WMS系统响应成功
        /// </summary>
        public override bool TransactionIsSuccess(HttpRespXml_ItemsSynchronize pResp)
        {
        ...
        override public string GetApiMethod()
        {
            return CWmsMisSystemParamCache.Cache.ApiMethod_ItemsSync?.PValue;
        }
        ...
        #region Product Ids from args
                if (null == args)
                {
                    C_WMS.Data.Utility.MyLog.Instance.Error("{0}.ParseArguments(params object[]), 传入非法参数", MethodBase.GetCurrentMethod().DeclaringType.Name);
                    err = TError.Post_NoParam.Int();
                }
                else if (0 == args.Length)
                {
                    ProductIdList = CWmsDataFactory.GetV_ProductIds(null).ToDictionary(itemCode => itemCode, itemCode => string.Empty);
                } // 同步全部商品
                else if (args[0] is string)
                {
                    ProductIdList = new Dictionary<string, string>() { { args[0] as string, string.Empty } };
                } // 同步单条商品
                else if (args[0] is List<string>)
                {
                    ProductIdList = (args[0] as List<string>).ToDictionary(id => id, id => "");
                } // 同步部分商品
                else
                {
                    C_WMS.Data.Utility.MyLog.Instance.Error("{0}.ParseArguments({1}), 不支持的参数类型: {2}", MethodBase.GetCurrentMethod().DeclaringType.Name, args[0], args[0].GetType());
                    err = TError.Post_ParamError.Int();
                } // 不支持的参数类型
        #endregion

        #region handle specifications of product, [itemCode]=[productid]-[specification-link-id]
                if (TError.RunGood.Int() == err)
                {
                    err = HandleSpecificationLinkId();
                } // if (TError.RunGood.Int() == err)
        #endregion
            ...
            {
                C_WMS.Data.Utility.MyLog.Instance.Warning(ex, "{0}.ParseArguments()发生异常.\r\nDEBUG INFO: {1}", MethodBase.GetCurrentMethod().DeclaringType.Name, Utility.CWmsDataUtility.GetDebugInfo_Args(args));
        ...
        {
            if (0 == ProductIdList?.Count)
            {
                C_WMS.Data.Utility.MyLog.Instance.Warning("{0}.Reset709()失败，ProductIdList={0}", MethodBase.GetCurrentMethod().DeclaringType.Name, ProductIdList);
                return TError.Pro_HaveNoData.Int();
            }
            else
                return Update709(TDict709_Value.EUpdateProduct, ProductIdList.Keys.ToList(), TDict285_Values.EDeleted, TDict285_Values.EUnknown);
        }
        ...
        {
            return Update709(TDict709_Value.EUpdateProduct, ProductIdList.Keys.ToList(), (pUpdateOK) ? TDict285_Values.ENormal : TDict285_Values.EDeleted, TDict285_Values.EUnknown);
        }
        ...
        /// </summary>
        /// <returns></returns>
        override public HttpReqXml_ItemsSyncronize NewRequestObj()
        {
            Dictionary<string, string> syncList = new Dictionary<string, string>(); // cache the products that ready for synchronizing.

            // get barcodes for all products to be synchronizing
            foreach (var product in ProductIdList)
            {
                CWmsInventoryMonitoring trans = new CWmsInventoryMonitoring();
                var resp = trans.DoTransaction(product.Key);
                if (null != resp && (resp.IsSuccess() || resp.message.Contains("商品不存在")))
                    syncList.Add(product.Key, (resp.IsSuccess()) ? resp.items[0].barcodeNum : product.Key);
                else
                    C_WMS.Data.Utility.MyLog.Instance.Warning("{0}.NewRequestObj(), 获取商品{1}的条码失败, RESPONSE={2}", MethodBase.GetCurrentMethod().DeclaringType.Name, product.Key, resp);
            }

            // init request object for each owner in OwnerList
            foreach (var owner in OwnerList)
            {
                var reqObj = new HttpReqXml_ItemsSyncronize();
                reqObj.actionType = "update"; // TODO: using SystemParam
                Params.PostParams.customerId = reqObj.ownerCode = owner.WOwner.WmsID;
                reqObj.warehouseCode = "OTHER";    // TODO: 统仓统配，传OTHER
                reqObj.items = syncList.Select(product => new HttpReqXml_ItemsSync_Item(CWmsDataFactory.NewProduct(product.Key))).ToList();
                ReqObjList.Add(reqObj);
            }
            return ReqObjList.First();
        }
        ...
        /// <returns>返回Response XML对应的数据实例</returns>
        override public HttpRespXml_ItemsSynchronize HandleResponse(byte[] respData)
        {
            try { return new HttpRespXml_ItemsSynchronize(Encoding.UTF8.GetString(respData)); }
            catch (Exception ex)
            {
        ...
        public override HttpRespXml_ItemsSynchronize DoTransaction(HttpReqXml_ItemsSyncronize pReqObj)
        {
            var response = new HttpRespXml_ItemsSynchronize();
            response.flag = "success";
            response.code = "200";
            response.message = "";
            foreach (var request in ReqObjList)
            {
                var tmpResp = base.DoTransaction(request);
                if (null == tmpResp)
                {
                    response.flag = "failure";
                    response.code = TError.Ser_ErrorPost.Int().ToString();
                    response.message = string.Format("Failed in synchronizing products of owner[{0}], WMS response null.", request?.ownerCode);
                }
                else if (!tmpResp.IsSuccess())
                {
                    response.flag = tmpResp.flag;
                    response.code = tmpResp.code;
                    response.message += tmpResp.message + "\r\n";
                    response.items.AddRange(tmpResp.items);
                }
                else response.items.AddRange(tmpResp.items);
                // TODO: send DingDing for operator
            }
            return response;
        }

        /// <summary>
        /// 处理待更新商品的规格关联ID。如果在ProductIdList中，存在有规格关联的商品，则将[ProductId]-[规格关联ID]作为新商品的ItemCode添加进ProductIdList中。
        /// 若方法执行成功则返回TError.RunGood；否则返回其他值。
        /// </summary>
        /// <returns>若方法执行成功则返回TError.RunGood；否则返回其他值。</returns>
        int HandleSpecificationLinkId()
        {
            try
            {
                Dictionary<string, string> ggLinkList = null;   // Dictionary<LinkId, ProductId> ggLinkList
                int err = CWmsDataFactory.GetV_SpecificationLink(out ggLinkList);
                if (null != ggLinkList && 0 < ggLinkList.Count)
                {
                    var itemDict = ggLinkList.Keys.Select(linkId => WmsProduct.NewItemCode(ggLinkList[linkId], linkId)).ToDictionary(itemCode => itemCode, itemCode => string.Empty);
                    foreach (var item in itemDict) ProductIdList.Add(item.Key, item.Value);
                }
                else
                    err = (null == ggLinkList) ? TError.WCF_RunError.Int() : TError.RunGood.Int(); // WCF查询获取所有规格关联ID数量为0时，不认为逻辑失败.
                ggLinkList.Clear();
            ...
            {
                C_WMS.Data.Utility.MyLog.Instance.Warning(ex, "CWmsTransactionItemSync.HandleSpecificationLinkId()发生异常.");
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        override public int ActivateImpl(params object[] args)
        {
            if (TAsyncStatus.EStopped == AsyncStatus)
            {
                _asyncArg = args?[0];
                AsyncStatus = TAsyncStatus.EBegin;
                return TError.RunGood.Int();
            }
            else return TError.Ser_EvenHaveData.Int();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="args"></param>
        override public int RunImpl(object sender, EventArgs args)
        {
            int err = TError.RunGood.Int();
            switch (AsyncStatus)
            {
                case TAsyncStatus.EBegin: { break; }         // 开始异步会话
                case TAsyncStatus.EGetPidList: { break; }    // get list of id of products to be synchronizing.
                case TAsyncStatus.EParseArguments:
                    {
                        err = ParseArguments(_asyncArg);
                        break;
                    }    // parse argument(list of id )
                case TAsyncStatus.EReset709:
                    {
                        err = Reset709();
                        break;
                    }// reset IsUpdateOK for all products
                case TAsyncStatus.ENewRequestObject: {
                        NewRequestObj();
                        break;
                    }// new an object of requset
                case TAsyncStatus.EDoTransaction: {
                        ResponseObject = DoTransaction(null);
                        break;
                    }// executing transaction
                case TAsyncStatus.EUpdate709: {
                        if (null != ResponseObject && ResponseObject.IsSuccess()) err = Update709(true);
                        else err = TError.Post_NoChange.Int();
                        break;
                    }// update 709 according to response from HTTP Transaction
                case TAsyncStatus.ESuccess: {

                        break;
                    }// WMS同步操作成功
                case TAsyncStatus.EFailed: { break; }// WMS同步操作失败
                case TAsyncStatus.EStopped: { break; }// 结束异步会话
                default: break;
            }
                    return err;
        }
    ...
    /// 异步通讯控制器
    /// </summary>
    class CWmsMethod_ItemsSyncAsyncCtrl
    {
        /// <summary>
        /// 异步状态
        /// </summary>
        ...
        /// default constructor
        /// </summary>
        public CWmsMethod_ItemsSyncAsyncCtrl() { }
        ...
        /// <param name="pHandler">数据处理实体</param>
        /// <returns>若仍要执行下一步一步操作，则应返回TError.RunGood；否则返回其他值</returns>
        public int DoRunL(CWmsMethod_ItemsSyncDataHandler pHandler)
        {
            int err = TError.WCF_RunError.Int();
            string errMsg = string.Empty;
    ...
    /// 异步通讯数据Handler
    /// </summary>
    class CWmsMethod_ItemsSyncDataHandler : IDisposable
    {
        #region Properties
        public List<CWmsProduct> ProductList { get { return mProductList; } }
        List<CWmsProduct> mProductList = null;
        ...
        /// default constructor
        /// </summary>
        public CWmsMethod_ItemsSyncDataHandler()
        {
            mProductList = new List<CWmsProduct>(5);
            mSyncOkProducList = new List<CWmsProduct>(1);
            mSyncFailedProducList = new List<CWmsProduct>(1);
            mPwiUpdateList = new List<Product_WMS_Interface>(5);
            mPwiAddList = new List<Product_WMS_Interface>(5);
            mOwnerList = new Dictionary<string, WmsOwner>(1);
        }
        ...
        public int GetReqBodyXml(out string pMsg)
        {
            ...
            {
                mCurrentRequest = new HttpReqXml_ItemsSyncronize();
                CurrentRequest.actionType = "update";// CWmsConsts.sStrApiItemsSyncActionTypeUpdate;
                CurrentRequest.ownerCode = CurrentOwner.WmsID;
                CurrentRequest.warehouseCode = "OTHER";    // 统仓统配，传OTHER
