...
namespace C_WMS.Data.Wms.Transaction
{
    ...
    /// 入库单创建接口通讯类
    /// </summary>
    class CWmsEntryOrderCreate : MWmsTransactionBase<CWmsEntryOrderCreate, HttpReqXml_EntryOrderCreate, HttpRespXml_EntryOrderCreate>
    {
#if C_WMS_V1
        ...
        {
            return new CWmsEntryOrderCreateImpl();
        }
#endif
    }
        ...
        /// 
        /// </summary>
    class CWmsEntryOrderCreateImpl : CWmsOrderTransactionImpl<HttpReqXml_EntryOrderCreate, HttpRespXml_EntryOrderCreate>
    {
        /// <summary>
        /// 返回API接口的method
        /// </summary>
        ...
        public override string GetApiMethod()
        {
            return CWmsMisSystemParamCache.Cache.ApiMethod_EntryOrderCreate?.PValue;
        }
        ...
        {
            var order = CWmsEntryOrderHandler.NewOrder(OrderId);
        ...
        /// </summary>
        /// <returns></returns>
        public override HttpReqXml_EntryOrderCreate NewRequestObj()
        {
            var order = CWmsEntryOrderHandler.NewOrder(OrderId);
            return new HttpReqXml_EntryOrderCreate(order);
        }
        ...
        public override HttpRespXml_EntryOrderCreate HandleResponse(byte[] respData)
        {
            if (null == respData)
            {
                C_WMS.Data.Utility.MyLog.Instance.Error("CWmsEntryOrderCreateImpl.HandleResponse(), 传入空引用参数byte[]");
                return null;
            }
            else
            {
                try { return new HttpRespXml_EntryOrderCreate(Encoding.UTF8.GetString(respData)); }
                catch (Exception ex)
                {
                    C_WMS.Data.Utility.MyLog.Instance.Error(ex, "CWmsEntryOrderCreateImpl.HandleResponse()发生异常. Response Length={0}", respData.Length);
                    return null;
                }
            }
        }
